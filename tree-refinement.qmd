...
![The default BipC tree output from `Simple Phylogeny`, visualised in `iToL`.](assets/images/itol-simphy-01.png){#figi-tol-simphy-01 width=80%}
...
The Neighbour-Joining methodology that was used to build the tree in `Simple Phylogeny` assigns a root to the tree, although this might not be the true phylogenetic root of the tree. We can inspect the tree without any assumption of root placement by examining the _unrooted_ tree.
...
To highlight a clade in `iToL` by changing branch colour and thickness, first select the ancestral branch to a clade, as in @fig-itol-clade-select.

![Selecting the recently-rooted clade in the `iTol` tree.](assets/images/itol-clade-select.png){#fig-itol-clade-select width=80%}

Then bring up the context menu and select `Branches -> Whole Clade` to see colour and line thickness options (@fig-itol-clade-context). Set those options to whatever seems like a good visualisation choice for you.
...
Any phylogenetic tree we generate reflects only the data used to construct it. If we use sequences corresponding to a single gene or gene produce, as we did for BipC, we generate a history **for that gene/gene product** - this is a _gene tree_.
...
**We cannot always assume that the history of a gene reflects the history of the organism it is currently found in.**
:::
...
- `9BURK`: _B. mayonis_, _B. humptydooensis_, _B. oklahomensis_, _Burkholderia sp. 
- `BURM7`: _B. mallei_
- `BURM9`: _B. mallei_
- `BURMA`: _B. mallei_
- `BURML`: _B. mallei_
